  ...
  <div>
    <el-row>
      ...
      <el-col :md="16" :xs="24" class="col-space">
          ...
          <el-col :xs="8" :sm="6" class="row-space">
            <el-tooltip content="折叠全部" placement="top">
              <el-select v-model="fold" placeholder="折叠全部" size="mini">
                <el-option v-for="item in options.fold"
                           :key="item.value"
                           :label="item.label"
                           :value="item.value"></el-option>
              </el-select>
          ...
          <el-col :xs="8" :sm="6" class="row-space">
            <el-tooltip content="图标风格" placement="top">
              <el-select v-model="iconStyle" placeholder="图标风格" size="mini">
                <el-option v-for="item in options.iconStyle"
                           :key="item.value"
                           :label="item.label"
                           :value="item.value"></el-option>
              </el-select>
          ...
          <el-col :xs="8" :sm="6" class="row-space">
            <el-tooltip content="字体大小" placement="top">
              <el-select v-model="fontSize" placeholder="字体大小" size="mini">
                <el-option v-for="item in 8"
                           :key="(item + 4) * 2"
                           :label="(item + 4) * 2"
                           :value="(item + 4) * 2"></el-option>
              </el-select>
            </el-tooltip>
          </el-col>
          ...
          <el-col :xs="8" :sm="6" class="row-space">
            <el-tooltip content="行间距" placement="top">
              <el-select v-model="lineHeight" placeholder="行间距" size="mini">
                <el-option v-for="item in 16"
                           :key="(item + 8) * 2"
                           :label="(item + 8) * 2"
                           :value="(item + 8) * 2"></el-option>
              </el-select>
            </el-tooltip>
          </el-col>
        ...
        <!-- json 预览 -->
        <el-row>
          <Json-view :data="json" :theme="theme" :deep="deep" :iconStyle="iconStyle" :fontSize="fontSize" :lineHeight="lineHeight" :closed="fold" :iconColor="iconColor"/>
        </el-row>
      </el-col>
    </el-row>
...
export default {
  ...
  data() {
    return {
      ...
      options: {
        themes: [{"label": "默认主题", "value": "default"}, {"label": "one-dark", "value": "one-dark"}, {"label": "vs-code", "value": "vs-code"}],
        fold: [{"label": "折叠", "value": true}, {"label": "不折叠", "value": false}],
        iconStyle: [{"label": "方形", "value": "square"}, {"label": "圆形", "value": "circle"}, {"label": "三角形", "value": "triangle"}],
      }
    }
  },
  methods: {

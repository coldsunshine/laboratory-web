          ...
          <el-tabs :value="activeName" @tab-click="handleClick" lazy>
            ...
            <el-tab-pane label="JSON" name="tab_json">
              <div class="text-center" style="margin: 10px 0 20px 0">
                <el-radio-group v-model="jsonStyle.checked" size="mini" @change="jsonStyleChange">
                  <el-radio-button v-for="item in jsonStyle.styles" :key="item.value" :label="item.value">{{ item.label }}</el-radio-button>
                </el-radio-group>
              </div>
...
export default {
  ...
  data() {
    return {
      ...
      jsonStyle: {checked: 'JSON', styles: [{"value": "JSON", "label": "样式一"}, {"value": "JSON_NEW", "label": "样式二"}]},
    };
  },
  methods: {
    handleClick(tab) {
